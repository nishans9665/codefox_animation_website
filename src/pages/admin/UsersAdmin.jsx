...
const UsersAdmin = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            const res = await axios.get(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/users`, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingUser) {
                // UPDATE USER
                await axios.put(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/users/${editingUser.id}`, formData, config);
                ...
                await axios.post(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/users`, formData, config);
    ...
    const handleDelete = async (id) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/users/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
