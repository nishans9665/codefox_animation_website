...
const ProjectsAdmin = () => {
    ...
    const fetchProjects = async () => {
        try {
            ...
            const res = await axios.get(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/projects`, {
    ...
    const openModal = (project = null) => {
        if (project) {
            ...
            setImagePreview(project.image.startsWith('http') ? project.image : `${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}${project.image}`);
            setImageFile(null);
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingProject) {
                await axios.put(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/projects/${editingProject.id}`, data, config);
                toast.success("Project updated successfully!");
            } else {
                if (!imageFile) throw new Error("Image is required for new projects");
                await axios.post(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/projects`, data, config);
    ...
    const handleDelete = async (id) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/projects/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
                    ...
                    <tbody>
                        {paginatedProjects.map(project => (
                            <tr key={project.id}>
                                <td style={{ width: '90px' }}>
                                    {project.image ? (
                                        <div className="post-thumbnail" style={{ backgroundImage: `url(${project.image.startsWith('http') ? project.image : `${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}${project.image}`})` }}></div>
                                    ) : (
