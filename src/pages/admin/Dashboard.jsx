        ...
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                const [contactsRes, postsRes, usersRes, projectsRes] = await Promise.all([
                    axios.get(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/contacts`, config).catch(() => ({data: []})),
                    axios.get(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/posts/admin`, config).catch(() => ({data: []})),
                    axios.get(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/users`, config).catch(() => ({data: []})),
                    axios.get(`${(import.meta.env.VITE_API_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000'))}/api/projects`, config).catch(() => ({data: []}))
                ]);
